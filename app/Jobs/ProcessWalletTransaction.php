...
class ProcessWalletTransaction implements ShouldQueue, ShouldBeUnique
{
    ...
    public function handle()
    {
        DB::transaction(function () {
            switch ($this->resource->getType()) {
                case "send":
                    return $this->handleOutgoingTransaction();
                case "receive":
                    return $this->handleIncomingTransaction();
            }
        });
        $this->cleanUp();
    }
    ...
    protected function consolidate(TransferRecord $record)
    {
        ...
        $adapter = $wallet->coin->adapter;
        if ($adapter instanceof Consolidates) {
            $adapter->consolidate($wallet->resource, $record->walletAddress->address);
        }
    }

...
class WalletController extends Controller
{
    ...
    public function consolidate(Request $request, $identifier)
    {
        ...
        if ($coin->adapter instanceof Consolidates) {
            try {
                $coin->adapter->consolidate($coin->wallet->resource, $validated['address']);
    ...
    protected function adapters()
    {
        return app('coin.adapters')
            ...
            ->filter(function ($adapter) {
                return $adapter instanceof Adapter && Coin::whereIdentifier($adapter->getIdentifier())->doesntExist();
            })
     ...
     * @return Coin
     */
    protected function coin(string $identifier)
    {

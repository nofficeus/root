...
class FeatureLimitController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $validated = $this->validate($request, [
            ...
            'limits.*.unverified_limit' => 'required|numeric|min:0',
            'limits.*.basic_limit'      => 'required|numeric|min:0',
            'limits.*.advanced_limit'   => 'required|numeric|min:0',
            'limits.*.period'           => 'required|in:day,month,year',
        ]);

        foreach ($validated['limits'] as $name => $data) {

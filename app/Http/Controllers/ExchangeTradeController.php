...
class ExchangeTradeController extends Controller
{
    ...
    public function sell(VerifiedRequest $request)
    {
        return Auth::user()->acquireLock(function (User $user) use ($request) {
            ...
            $record = $account->acquireLock(function (WalletAccount $account) use ($request, $user) {
                ...
                $exchangeTrade = new ExchangeTrade(['type' => 'sell']);
                ...
                DB::transaction(function () use ($exchangeTrade, $limit, $user) {
                    ...
                    $exchangeTrade->paymentAccount->credit($exchangeTrade->payment_value, $description);

                    $traderAccount->transferRecords()->create([
                        'type'         => 'receive',
                        'dollar_price' => $exchangeTrade->dollar_price,
                    ...
                    Earning::wallet($exchangeTrade->getFeeValueObject(), $description, $exchangeTrade->trader);

                    $limit->setUsage($exchangeTrade->payment_value, $user);
                });

                return tap($exchangeTrade)->update(['completed_at' => now()]);
            });
    ...
    public function calculateBuy(Request $request)
    {
        ...
        return [
            'deductible' => $amount->add($exchangeFee)->getPrice(Auth::user()->currency),
            'fee'        => $exchangeFee->getPrice(Auth::user()->currency),
        ];
    }
    ...
    public function buy(VerifiedRequest $request)
    {
        return Auth::user()->acquireLock(function (User $user) use ($request) {
            ...
            $record = $paymentAccount->acquireLock(function (PaymentAccount $paymentAccount) use ($request, $user) {
                ...
                if ($paymentAccount->available->lessThan($deductible)) {
                ...
                $exchangeTrade = new ExchangeTrade(['type' => 'buy']);

    ...
    protected function updateSupportedCurrencyStatistics()
    {
        SupportedCurrency::all()->each(function (SupportedCurrency $currency) {
            ...
            $currency->statistic()->firstOrNew()->fill([
                'balance_on_trade' => $paymentAccounts->sum(function (PaymentAccount $account) {
                    return $account->balance_on_trade->getValue();
                }),
                'balance'          => $paymentAccounts->sum(function (PaymentAccount $account) {
                    return $account->balance->getValue();
                }),
            ])->save();
        });
    }

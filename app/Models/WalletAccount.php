...
//            if ($record->getAvailableObject()->isNegative()) {
//                SystemLog::warning("Unexpected negative {$record->wallet->coin->name} balance of {$record->user->name}");
//            }
//        });
    }
    ...
    {
        $record = $this->acquireLock(function () use ($amount, $target) {
            $account = $this->fresh();
            ...
            if ($target instanceof WalletAccount) {
                ...
                if ($operatorAccount && $withdrawalFee->isPositive() && $account->isNot($operatorAccount)) {
                    $operatorAccount->transferRecords()->create([
                        'type'         => 'receive',
                        'value'        => $withdrawalFee->getAmount(),
                        'description'  => $description = "Withdrawal Fee: {$account->user->name} - {$record->id}",
                        'dollar_price' => $coin->getDollarPrice(),
                    ]);
                    Earning::wallet($withdrawalFee, $description, $operatorAccount->user);
                }
